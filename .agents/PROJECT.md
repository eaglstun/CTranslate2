...
- The forward pass (GEMM via MPS, softmax, norms, rotary, gather, transpose, bias+activation, mul, sampling, fused decode attention) runs on the GPU in fp32 and fp16; a full transformer runs end-to-end matching CPU output. Conv and a few general-axis variants run via the CPU reference. Perf state: ops commit asynchronously (per-op `waitUntilCompleted` removed) and MPS GEMM objects are cached. On a real LLM (Qwen2.5-0.5B) Metal **wins every measured regime**: GEMM-heavy prefill (fp16 batch-8 ~2.6× faster than CPU) and — since the fused single-launch decode-attention kernel (M16) — autoregressive decode too (bs=1 edges CPU, bs=8 ~2.8×). **Whisper beam-search decode also now wins (M17): large-v3 fp16 6.4× realtime, 4.3× faster than CPU, transcript byte-identical** — the fix was native Metal `Transpose` and a row-parallel `gather` kernel (a CPU-reference `Transpose` had been draining the GPU queue ~128×/token in beam decode). The per-op GPU-API floor is real but is attacked by _fewer, bigger ops_, NOT commit count (command-buffer reuse was tried and reverted: it killed CPU/GPU overlap). Two env-gated profilers help find these: `CT2_AUTO_PROFILE=cpu|metal` and `CT2_METAL_STATS=1`. See `METAL_BENCHMARKS.md`.
