...
- The forward pass (GEMM via MPS, softmax, norms, rotary, gather, bias+activation, mul, sampling, fused decode attention) runs on the GPU in fp32 and fp16; a full transformer runs end-to-end matching CPU output. Conv and a few general-axis variants run via the CPU reference. Perf state: ops commit asynchronously (per-op `waitUntilCompleted` removed) and MPS GEMM objects are cached. On a real LLM (Qwen2.5-0.5B) Metal **wins every measured regime**: GEMM-heavy prefill (fp16 batch-8 ~2.6× faster than CPU) and — since the fused single-launch decode-attention kernel (M16) — autoregressive decode too (bs=1 edges CPU, bs=8 ~2.8×). The per-op GPU-API floor is real but is attacked by _fewer, bigger ops_, NOT commit count (command-buffer reuse was tried and reverted: it killed CPU/GPU overlap). See `METAL_BENCHMARKS.md`.
