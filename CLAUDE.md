...
- The forward pass (GEMM via MPS, softmax, norms, rotary, gather, bias+activation, mul) runs on the GPU in fp32 and fp16; a full transformer runs end-to-end matching CPU output. Sampling/conv/etc. run via the CPU reference. Perf state: ops commit asynchronously (per-op `waitUntilCompleted` removed) and MPS GEMM objects are cached. On a real LLM (Qwen2.5-0.5B) Metal **wins** the GEMM-heavy prefill (fp16 batch-8 ~2.6× faster than CPU) but **loses** tiny-op autoregressive decode to the fundamental per-op GPU-API floor — which is NOT commit count (command-buffer reuse was tried and reverted: it killed CPU/GPU overlap). See `METAL_BENCHMARKS.md`.
